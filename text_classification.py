...
def main(args):
    ...
    if args.dataset == "mnli":
        ...
        text_key = (
            "text" if (args.dataset in ["ag_news", "imdb", "yelp"]) else "sentence"
        )
    ...
    torch.save(
        ...
        os.path.join(
            args.result_folder,
            "%s_%s%s.pth" % (args.model.replace("/", "-"), args.dataset, suffix),
        ),
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tqdm", default=True, type=bool_flag, help="Use tqdm in output"
    )

    # Data
    parser.add_argument(
        "--data_folder", required=True, type=str, help="folder in which to store data"
    )
    parser.add_argument(
        "--dataset",
        default="dbpedia14",
    ...
    # Model
    parser.add_argument("--model", default="gpt2", type=str, help="type of model")
